...
const toCamelCase = <T extends Record<string, unknown> | readonly unknown[]>(
  obj: T,
...
const fetchTransaction = (hash: string): Promise<Transaction | null> =>
  bitcoin.transactions
    ...
    .then(toCamelCase)
    .then(function (data: Transaction) {
      if (!data) return null;
...
export const useGetBtcBlockByTransaction = function (
  ...
  return { error, isLoading, transactions: sortedTransactions };
};
